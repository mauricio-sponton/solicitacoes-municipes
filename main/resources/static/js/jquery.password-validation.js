	...
	$.fn.extend({
		passwordValidation: function(_options, _callback, _confirmcallback) {
			...
			function buildPasswordRegex() {
				...
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.upperCaseSet + "][^" + CHARSETS.upperCaseSet + "]*").repeat(_options.minUpperCase) + ")", 	"message": "A senha deve conter pelo menos " + _options.minUpperCase + " letra maiúscula."});
				if(_options.noLower) 	cases.push({"regex": "(?=" + CHARSETS.lowerCaseSet + ")",  																				"message": "Password can't contain a Lower Case Letter"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.lowerCaseSet + "][^" + CHARSETS.lowerCaseSet + "]*").repeat(_options.minLowerCase) + ")", 	"message": "A senha deve conter pelo menos " + _options.minLowerCase + " letra minúscula."});
				if(_options.noDigit) 	cases.push({"regex": "(?=" + CHARSETS.digitSet + ")", 																					"message": "Password can't contain a Number"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.digitSet + "][^" + CHARSETS.digitSet + "]*").repeat(_options.minDigits) + ")", 			"message": "A senha deve conter pelo menos " + _options.minDigits + " dígito."});
				if(_options.noSpecial) 	cases.push({"regex": "(?=" + CHARSETS.specialSet + ")", 																				"message": "Password can't contain a Special Character"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.specialSet + "][^" + CHARSETS.specialSet + "]*").repeat(_options.minSpecial) + ")", 		"message": "A senha deve conter pelo menos " + _options.minSpecial + " caracter especial."});

				cases.push({"regex":"[" + charsetToString() + "]{" + _options.minLength + ",}", "message":"A senha deve conter pelo menos " + _options.minLength + " caracteres."});
			...
			function validateField() {
				...
				if(_options.failRepeats && $(_element).val().search(new RegExp("(.)" + (".*\\1").repeat(_options.maxRepeats - 1), "gi")) != -1) {
					failedCases.push("A senha não pode conter " + _options.maxRepeats + " caracteres repetidos.");
				}
				if(_options.failConsecutive && $(_element).val().search(new RegExp("(?=(.)" + ("\\1").repeat(_options.maxConsecutive) + ")", "g")) != -1) {
					failedCases.push("A senha não pode conter o mesmo caracter mais do que " + _options.maxConsecutive + " vezes seguidas.");
				}
